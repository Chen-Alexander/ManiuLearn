...
import com.maniu.weblib.command.CommandsManager;
import com.maniu.weblib.interfaces.Action;
...
/**
 * WebView所有请求分发  命令分发  是被WebView中网页传递过来的命令  然后做出相对应的动作
 *
 * 规则：
 *
 * 1、先处理UI依赖
...
public class CommandDispatcher {
     ...
     * @param commandLevel
     * @param cmd
     * @param params 网页传过来的参数  要给native层交互的内容
     * @param webView
    ...
    private void execUI(final Context context,final  int commandLevel,final  String cmd, final String params,
        ...
        Log.e("CommandDispatcher", "execUI on " + Process.myPid() + "->" + Thread.currentThread().getName());
    ...
    private void execNonUI(Context context, int commandLevel, String cmd, String params,final  WebView webView, final DispatcherCallBack dispatcherCallBack) throws Exception {
        ...
        if (SystemInfoUtil.inMainProcess(context, android.os.Process.myPid())) {
            Log.e("CommandDispatcher", "execNonUI on " + Process.myPid() + "->" + Thread.currentThread().getName());
